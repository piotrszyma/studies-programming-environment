...
for i in `svn log -v --xml $1 | grep -B 1 '<author>'$2'</author>' | awk 'NR % 3 == 1' | grep -Po '\d+'`; do
  ...
  for ADDED_PER_COMMIT in `svn diff https://156.17.7.16/p_25/  -c $REV | diffstat -t | awk -F "," '{print $1}' | awk 'NR>1'`; do
    SUM_ADDED=`expr $SUM_ADDED + $ADDED_PER_COMMIT`
  done
  
  for DELETED_PER_COMMIT in `svn diff https://156.17.7.16/p_25/  -c $REV | diffstat -t | awk -F "," '{print $2}' | awk 'NR>1'`; do
